...
public class CoreCalculator {
     ...
     * 计算命盘
     */
    public static Astrolabe calculate(int lunarYear, int lunarMonth, int lunarDay, int birthHour, int gender) {
        // 验证输入
        validateInput(lunarYear, lunarMonth, lunarDay, birthHour, gender);
        ...
        try {
            ...
            // 设置性别和时辰
            astrolabe.setGender(gender == 1 ? "男" : "女");
     ...
     * 验证输入参数
     */
    private static void validateInput(int lunarYear, int lunarMonth, int lunarDay, int birthHour, int gender) {
        ...
        if (gender != 1 && gender != 2) {
            throw new BusinessException(ResultCode.PARAM_ERROR, "性别只能是1或2");
        }
    }
