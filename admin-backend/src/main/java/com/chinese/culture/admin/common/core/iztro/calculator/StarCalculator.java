...
public class StarCalculator {
     ...
     * 计算紫微星系星耀位置
     */
    public static List<StarBO> calculateZiWeiStars(int lunarYear, int lunarMonth, int lunarDay, int birthHour) {
        ...
        // 计算天机星位置（紫微顺三）
        StarBO tianJi = new StarBO(StarName.TIANJI, StarType.MAJOR);
        tianJi.setPosition((ziWeiPosition + 3) % 12);
        stars.add(tianJi);

        // 计算太阳星位置（天机顺六）
        StarBO taiYang = new StarBO(StarName.TAIYANG, StarType.MAJOR);
        taiYang.setPosition((ziWeiPosition + 9) % 12);
        stars.add(taiYang);

        // 计算武曲星位置（太阳顺四）
        StarBO wuQu = new StarBO(StarName.WUQU, StarType.MAJOR);
        wuQu.setPosition((ziWeiPosition + 1) % 12);
        stars.add(wuQu);

        // 计算天同星位置（武曲顺一）
        StarBO tianTong = new StarBO(StarName.TIANTONG, StarType.MAJOR);
        tianTong.setPosition((ziWeiPosition + 2) % 12);
        stars.add(tianTong);

        // 计算廉贞星位置（天同顺六）
        StarBO lianZhen = new StarBO(StarName.LIANZHEN, StarType.MAJOR);
     ...
     * 计算天府星系星耀位置
     */
    public static List<StarBO> calculateTianFuStars(int lunarYear, int lunarMonth, int lunarDay, int birthHour) {
        ...
        // 计算太阴星位置（天府逆四）
        StarBO taiYin = new StarBO(StarName.TAIYIN, StarType.MAJOR);
        taiYin.setPosition((tianFuPosition + 8) % 12);
        stars.add(taiYin);

        // 计算贪狼星位置（太阴逆二）
        StarBO tanLang = new StarBO(StarName.TANLANG, StarType.MAJOR);
        tanLang.setPosition((tianFuPosition + 6) % 12);
        stars.add(tanLang);

        // 计算巨门星位置（贪狼逆二）
        StarBO juMen = new StarBO(StarName.JUMEN, StarType.MAJOR);
        juMen.setPosition((tianFuPosition + 4) % 12);
        stars.add(juMen);

        // 计算天相星位置（巨门逆三）
        StarBO tianXiang = new StarBO(StarName.TIANXIANG, StarType.MAJOR);
        tianXiang.setPosition((tianFuPosition + 1) % 12);
        stars.add(tianXiang);

        // 计算天梁星位置（天相逆三）
        StarBO tianLiang = new StarBO(StarName.TIANLIANG, StarType.MAJOR);
        tianLiang.setPosition((tianFuPosition + 10) % 12);
        stars.add(tianLiang);

        // 计算七杀星位置（天梁逆三）
        StarBO qiSha = new StarBO(StarName.QISHA, StarType.MAJOR);
        qiSha.setPosition((tianFuPosition + 7) % 12);
        stars.add(qiSha);

        // 计算破军星位置（七杀逆三）
        StarBO poJun = new StarBO(StarName.POJUN, StarType.MAJOR);
     ...
     * 计算其他星耀位置
     */
    public static List<StarBO> calculateOtherStars(int lunarYear, int lunarMonth, int lunarDay, int birthHour) {
     ...
     * 计算紫微星系
     */
    private static List<StarBO> calculateZiWeiStars(int fiveElementsClass, int timeIndex) {
     ...
     * 计算天府星系
     */
    private static List<StarBO> calculateTianFuStars(EarthlyBranch ziWeiLocation) {
     ...
     * 计算其他星耀
     */
    private static List<StarBO> calculateOtherStars(EarthlyBranch monthBranch) {
