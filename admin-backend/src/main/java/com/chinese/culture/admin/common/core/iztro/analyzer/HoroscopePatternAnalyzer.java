...
import com.chinese.culture.admin.common.core.iztro.calculator.BrightnessCalculator;
...
public class HoroscopePatternAnalyzer {
     ...
     * @return 格局判定结果
     */
    public static Map<String, Object> judgePattern(List<PalaceBO> palaces) {
     ...
     * 检查紫微命格
     */
    private static void checkZiweiPattern(List<PalaceBO> palaces, List<Map<String, Object>> patterns) {
     ...
     * 检查天府命格
     */
    private static void checkTianfuPattern(List<PalaceBO> palaces, List<Map<String, Object>> patterns) {
        PalaceBO mingGong = findMingGong(palaces);
     ...
     * 检查禄存命格
     */
    private static void checkLucunPattern(List<PalaceBO> palaces, List<Map<String, Object>> patterns) {
        PalaceBO mingGong = findMingGong(palaces);
     ...
     * 检查文昌命格
     */
    private static void checkWenchangPattern(List<PalaceBO> palaces, List<Map<String, Object>> patterns) {
        PalaceBO mingGong = findMingGong(palaces);
    ...
    private static void checkFullMutagenPattern(List<PalaceBO> palaces, List<Map<String, Object>> patterns) {
        PalaceBO mingGong = findMingGong(palaces);
        if (mingGong != null && hasMutagens(mingGong, Arrays.asList(Mutagen.LU, Mutagen.QUAN, Mutagen.KE))) {
            Map<String, Object> pattern = new HashMap<>();
     ...
     * 检查破格
     */
    private static void checkBrokenPattern(List<PalaceBO> palaces, List<Map<String, Object>> patterns) {
        PalaceBO mingGong = findMingGong(palaces);
     ...
     * 判断宫位是否包含指定星耀
     */
    private static boolean hasStarByName(PalaceBO palace, String starName) {
     ...
     * 判断宫位是否包含指定四化
     */
    private static boolean hasMutagens(PalaceBO palace, List<Mutagen> mutagens) {
        Set<Mutagen> palaceMutagens = new HashSet<>();
        for (StarBO star : palace.getAllStars()) {
            palaceMutagens.addAll(star.getMutagenInfo().getMutagens());
//            palaceMutagens.addAll(star.getMutagens());
     ...
     * 判断是否有三奇贵人星
     */
    private static boolean hasThreeNobleStars(PalaceBO palace) {
     ...
     * 判断是否有四煞星
     */
    private static boolean hasFourEvilStars(PalaceBO palace) {
     ...
     * 判断是否有红艳星
     */
    private static boolean hasRedBeautyStars(PalaceBO palace) {
     ...
     * 判断是否有富贵星
     */
    private static boolean hasWealthNobleStars(PalaceBO palace) {
     ...
     * 判断是否为破格
     */
    private static boolean hasBrokenPattern(PalaceBO palace) {
     ...
     * 获取紫微命格等级
     */
    private static String getZiweiPatternLevel(PalaceBO palace) {
        StarBO ziwei = palace.getAllStars().stream()
     ...
     * 获取天府命格等级
     */
    private static String getTianfuPatternLevel(PalaceBO palace) {
        StarBO tianfu = palace.getAllStars().stream()
     ...
     * 获取禄存命格等级
     */
    private static String getLucunPatternLevel(PalaceBO palace) {
        StarBO lucun = palace.getAllStars().stream()
            .filter(s -> s.getName().equals("禄存"))
            .findFirst()
            .orElse(null);

        if (lucun != null && !lucun.getMutagenInfo().getMutagens().isEmpty()) {
            return "上格";
        }
        return "中格";
    }
     ...
     * 获取文昌命格等级
     */
    private static String getWenchangPatternLevel(PalaceBO palace) {
