...
public class PalaceAuspiciousnessCalculator {
     ...
     * @return 吉凶评分（0-100）
     */
    public static int calculateAuspiciousness(PalaceBO palace) {
        int score = 50; // 基础分
     ...
     * 计算星耀权重
     */
    private static int calculateStarWeights(PalaceBO palace) {
        int weight = 0;
     ...
     * 计算亮度影响
     */
    private static int calculateBrightnessEffect(PalaceBO palace) {
        int effect = 0;

        for (StarBO star : palace.getMajorStars()) {
            Brightness brightness = star.getBrightnessInfo() != null ?
                (star.getBrightnessInfo().getBrightness()) : Brightness.WANG;
     ...
     * 计算四化影响
     */
    private static int calculateMutagenEffect(PalaceBO palace) {
        int effect = 0;
     ...
     * 计算宫位本身属性影响
     */
    private static int calculatePalaceEffect(PalaceBO palace) {
        int effect = 0;
     ...
     * 判断是否为吉星
     */
    private static boolean isAuspiciousStar(StarBO star) {
     ...
     * 判断是否为煞星
     */
    private static boolean isMaleficStar(StarBO star) {
     ...
     * 获取宫位吉凶详细分析
     */
    public static Map<String, Object> getDetailedAnalysis(PalaceBO palace) {
        ...
        for (StarBO star : palace.getAllStars()) {
            if (AUSPICIOUS_WEIGHTS.containsKey(star.getName().getDescription())) {
                auspiciousStars.add(star.getName().getDescription());
            }
        }
        ...
        for (StarBO star : palace.getAllStars()) {
            if (INAUSPICIOUS_WEIGHTS.containsKey(star.getName().getDescription())) {
                inauspiciousStars.add(star.getName().getDescription());
            }
        }
        ...
        for (StarBO star : palace.getAllStars()) {
            for (Mutagen mutagen : star.getMutagenInfo().getMutagens()) {
                mutagens.computeIfAbsent(mutagen.getDescription(), k -> new ArrayList<>())
                    .add(star.getName().getDescription());
            }
        }
