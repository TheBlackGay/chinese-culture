...
public class AstrolabeAnalyzer {
     ...
     * @return 格局描述
     */
    public static String analyzeMingZhuPattern(AstrolabeBO astrolabe) {
        ...
        try {
            ...
            // 获取命宫
            PalaceBO mingGong = findMingGong(astrolabe.getPalaces());
            if (mingGong == null) {
     ...
     * 验证命盘数据完整性
     */
    private static void validateAstrolabeData(AstrolabeBO astrolabe) {
     ...
     * 分析命宫星耀组合
     */
    private static List<String> analyzeMingGongStars(PalaceBO mingGong) {

        List<String> patterns = new ArrayList<>();
        List<StarBO> stars = mingGong.getStars();
     ...
     * 分析三方四正
     */
    private static List<String> analyzeTrineAndOpposition(List<PalaceBO> palaces, PalaceBO mingGong) {
     ...
     * 分析四化
     */
    private static List<String> analyzeMutagens(PalaceBO mingGong) {
     ...
     * 统计吉星数量
     */
    private static int countGoodStars(List<StarBO> stars) {
     ...
     * @return 运势描述
     */
    public static String analyzeDecadalFortune(HoroscopeBO horoscope) {
        ...
        try {
            ...
            HoroscopeBO.DecadalHoroscope decadal = horoscope.getDecadal();

            // 添加大限基本信息
            int startAge = decadal.getStartAge();
            int endAge = decadal.getEndAge();
            List<StarBO> decadalStars = decadal.getStars();
            ...
            if (decadalStars != null && !decadalStars.isEmpty()) {
                fortune.append("主要星耀：").append(String.join("、", decadalStars.stream().map(StarBO::getName)
                        .map(StarName::getDescription)
                        .collect(Collectors.toList())));
            }
     ...
     * @return 运势描述
     */
    public static String analyzeYearlyFortune(HoroscopeBO horoscope) {
        ...
        try {
            ...
            StringBuilder fortune = new StringBuilder();
            HoroscopeBO.YearlyHoroscope yearly = horoscope.getYearly();

            // 添加流年基本信息
            int yearlyAge = yearly.getAge();
            List<StarBO> yearlyStars = yearly.getStars();
            ...
            if (yearlyStars != null && !yearlyStars.isEmpty()) {
                fortune.append("主要星耀：").append(String.join("、", yearlyStars.stream().map(StarBO::getName)
                        .map(StarName::getDescription)
                        .collect(Collectors.toList())));
            }
     ...
     * 分析运势等级
     */
    private static String analyzeFortuneLevel(List<StarBO> stars, List<String> mutagens) {
        ...
        if (stars != null) {
            for (StarBO star : stars) {
                String starName = star.getName().getDescription();
    ...
    private static List<String> getMutagens(HoroscopeBO.DecadalHoroscope decadal) {
    ...
    private static List<String> getYearlyDecStars(HoroscopeBO.YearlyHoroscope yearly) {
    ...
    private static List<String> getMutagens(HoroscopeBO.YearlyHoroscope yearly) {
