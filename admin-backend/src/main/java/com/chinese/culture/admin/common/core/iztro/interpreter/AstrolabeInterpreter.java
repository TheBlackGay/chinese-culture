...
import com.chinese.culture.admin.common.core.iztro.analyzer.PatternAnalyzer;
...
public class AstrolabeInterpreter {
     ...
     * @return 命盘解释结果
     */
    public static AstrolabeInterpretationBO interpretAstrolabe(AstrolabeBO astrolabe) {
        try {
            // 验证输入数据
            validateInput(astrolabe);

            AstrolabeInterpretationBO interpretation = new AstrolabeInterpretationBO();
     ...
     * 验证输入数据
     */
    private static void validateInput(AstrolabeBO astrolabe) {
        if (astrolabe == null) {
     ...
     * 解释宫位关系
     */
    private static AstrolabeInterpretationBO.PalaceRelations interpretPalaceRelations(List<PalaceBO> palaces) {
        AstrolabeInterpretationBO.PalaceRelations relations = new AstrolabeInterpretationBO.PalaceRelations();
     ...
     * 解释星耀组合
     */
    private static AstrolabeInterpretationBO.StarCombinations interpretStarCombinations(List<StarBO> stars) {
        AstrolabeInterpretationBO.StarCombinations combinations = new AstrolabeInterpretationBO.StarCombinations();
     ...
     * 解释四化关系
     */
    private static AstrolabeInterpretationBO.MutagenRelations interpretMutagenRelations(String yearStem, List<StarBO> stars) {
        AstrolabeInterpretationBO.MutagenRelations relations = new AstrolabeInterpretationBO.MutagenRelations();
        ...
        Map<String, Integer> starPositions = stars.stream()
                .collect(Collectors.toMap(it->it.getName().getDescription(), StarBO::getPosition));
     ...
     * 解释重要格局
     */
    private static List<String> interpretMajorPatterns(AstrolabeBO astrolabe) {
     ...
     * 分析命主格局
     */
    private static void analyzeMasterPattern(AstrolabeBO astrolabe, List<String> patterns) {
        PalaceBO masterPalace = astrolabe.getPalaces().get(0); // 命宫
        List<StarBO> masterStars = masterPalace.getStars();
     ...
     * 分析财帛格局
     */
    private static void analyzeWealthPattern(AstrolabeBO astrolabe, List<String> patterns) {
        // 查找财帛宫
        PalaceBO wealthPalace = astrolabe.getPalaces().stream()
                .filter(palace -> "财帛".equals(palace.getName()))
                .findFirst()
                .orElse(null);

        if (wealthPalace != null) {
            List<StarBO> wealthStars = wealthPalace.getStars();
     ...
     * 分析官禄格局
     */
    private static void analyzeCareerPattern(AstrolabeBO astrolabe, List<String> patterns) {
        // 查找官禄宫
        PalaceBO careerPalace = astrolabe.getPalaces().stream()
                .filter(palace -> "官禄".equals(palace.getName()))
                .findFirst()
                .orElse(null);

        if (careerPalace != null) {
            List<StarBO> careerStars = careerPalace.getStars();
     ...
     * 分析婚姻格局
     */
    private static void analyzeMarriagePattern(AstrolabeBO astrolabe, List<String> patterns) {
        // 查找夫妻宫
        PalaceBO marriagePalace = astrolabe.getPalaces().stream()
                .filter(palace -> "夫妻".equals(palace.getName()))
                .findFirst()
                .orElse(null);

        if (marriagePalace != null) {
            List<StarBO> marriageStars = marriagePalace.getStars();
    ...
    private static void analyzeWisdomPattern(AstrolabeBO astrolabe, List<String> patterns) {
        List<PalaceBO> palaces = astrolabe.getPalaces();
        PalaceBO mingGong = palaces.get(0);  // 命宫
    ...
    private static void analyzeAcademicPattern(AstrolabeBO astrolabe, List<String> patterns) {
        List<PalaceBO> palaces = astrolabe.getPalaces();
    ...
    private static void analyzeEmotionalPattern(AstrolabeBO astrolabe, List<String> patterns) {
        List<PalaceBO> palaces = astrolabe.getPalaces();
        PalaceBO qingGong = null;  // 情感宫
