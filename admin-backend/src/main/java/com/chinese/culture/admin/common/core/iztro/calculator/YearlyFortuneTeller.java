...
public class YearlyFortuneTeller {
     ...
     * @return 运势预测结果
     */
    public static Map<String, Object> predictYearlyFortune(List<PalaceBO> palaces, int currentYear,
                                                           int birthYear, boolean gender) {
        Map<String, Object> prediction = new HashMap<>();

        // 1. 获取流年宫位
        PalaceBO yearlyPalace = findYearlyFlowPalace(palaces);
     ...
     * 查找流年宫位
     */
    private static PalaceBO findYearlyFlowPalace(List<PalaceBO> palaces) {
        return palaces.stream()
            .filter(PalaceBO::isCurrentYearlyFlow)
            .findFirst()
            .orElse(null);
    }
     ...
     * 分析流年宫位
     */
    private static Map<String, Object> analyzePalaceForYear(PalaceBO yearlyPalace) {
        Map<String, Object> analysis = new HashMap<>();

        // 获取流年宫位吉凶分析
        PalaceAuspiciousnessAnalyzer.PalaceAuspiciousnessResult auspiciousness = PalaceAuspiciousnessAnalyzer.analyze(yearlyPalace);
     ...
     * 分析流年四化
     */
    private static Map<String, Object> analyzeMutagenForYear(List<PalaceBO> palaces, HeavenlyStem yearlyStem) {
        Map<String, Object> analysis = new HashMap<>();
     ...
     * 分析大小限
     */
    private static Map<String, Object> analyzeLimitsForYear(List<PalaceBO> palaces, int birthYear,
        ...
        // 获取当前大限宫位
        PalaceBO majorLimitPalace = palaces.stream()
            .filter(PalaceBO::isCurrentMajorLimit)
            .findFirst()
            .orElse(null);

        if (majorLimitPalace != null) {
            PalaceAuspiciousnessAnalyzer.PalaceAuspiciousnessResult majorLimitAnalysis = PalaceAuspiciousnessAnalyzer.analyze(majorLimitPalace);
            analysis.put("majorLimitAnalysis", majorLimitAnalysis);
        }

        // 获取当前小限宫位
        PalaceBO minorLimitPalace = palaces.stream()
            .filter(PalaceBO::isCurrentMinorLimit)
            .findFirst()
            .orElse(null);

        if (minorLimitPalace != null) {
            PalaceAuspiciousnessAnalyzer.PalaceAuspiciousnessResult minorLimitAnalysis = PalaceAuspiciousnessAnalyzer.analyze(minorLimitPalace);
     ...
     * 生成流年运势解释
     */
    private static String generateYearlyInterpretation(PalaceBO yearlyPalace,
    ...
    private static int getPalaceIndex(PalaceBO palace) {
