...
public class CoreCalculator {
     ...
     * 计算命盘
     */
    public static AstrolabeBO calculate(int lunarYear, int lunarMonth, int lunarDay, int birthHour, String genderStr) {
        // 验证输入
        ...
        try {
            ...
            // 创建十二宫
            List<PalaceBO> palaces = createTwelvePalaces(mingGongPosition);
            astrolabe.setPalaces(palaces);

            // 计算星耀位置
            List<StarBO> stars = calculateStarPositions(lunarYear, lunarMonth, lunarDay, mingGongPosition);
     ...
     * 创建十二宫
     */
    private static List<PalaceBO> createTwelvePalaces(int mingGongPosition) {
     ...
     * 计算星耀位置
     */
    private static List<StarBO> calculateStarPositions(int year, int month, int day, int mingGongPosition) {
     ...
     * 分配星耀到宫位
     */
    private static void distributeStars(List<StarBO> stars, List<PalaceBO> palaces) {
        for (StarBO star : stars) {
            int position = star.getPosition();
            if (position >= 0 && position < 12) {
                PalaceBO palace = palaces.get(position);
                star.setBranch(EarthlyBranch.fromDescription(palace.getBranch()));
     ...
     * 计算四化
     */
    private static void calculateTransformations(AstrolabeBO astrolabe) {
        ...
        for (PalaceBO palace : palaces) {
            for (StarBO star : palace.getStars()) {
                List<String> transformation = transformations.get(star.getName().getDescription());
