...
public class MutagenConflictDetector {
     ...
     * @return 冲突检测结果
     */
    public static Map<String, Object> detectConflicts(Star star, Palace palace) {
        Map<String, Object> result = new HashMap<>();
        List<Map<String, Object>> conflicts = new ArrayList<>();
        
        // 获取星耀的四化
        List<Mutagen> mutagens = star.getMutagens();
        if (mutagens.isEmpty()) {
        ...
        // 检查同宫其他星耀的四化
        List<Star> otherStars = palace.getAllStars().stream()
            .filter(s -> !s.equals(star))
            .collect(java.util.stream.Collectors.toList());
            
        for (Star otherStar : otherStars) {
            List<Mutagen> otherMutagens = otherStar.getMutagens();
     ...
     * @return 冲突检测结果
     */
    public static Map<String, Object> detectPalaceConflicts(Palace palace) {
        Map<String, Object> result = new HashMap<>();
     ...
     * 生成冲突描述
     */
    private static String generateConflictDescription(Star star1, Mutagen mutagen1, 
                                                    Star star2, Mutagen mutagen2) {
    ...
    public static int getConflictSeverity(Mutagen mutagen1, Mutagen mutagen2) {
        // 禄化与忌化的冲突最为严重
        if ((mutagen1 == Mutagen.LUCKY && mutagen2 == Mutagen.WEAK) ||
            (mutagen1 == Mutagen.WEAK && mutagen2 == Mutagen.LUCKY)) {
            return 100;
        }
        
        // 权化与忌化的冲突次之
        if ((mutagen1 == Mutagen.POWER && mutagen2 == Mutagen.WEAK) ||
            (mutagen1 == Mutagen.WEAK && mutagen2 == Mutagen.POWER)) {
            return 80;
        }
        
        // 科化与忌化的冲突再次
        if ((mutagen1 == Mutagen.SKILL && mutagen2 == Mutagen.WEAK) ||
            (mutagen1 == Mutagen.WEAK && mutagen2 == Mutagen.SKILL)) {
            return 60;
        }
