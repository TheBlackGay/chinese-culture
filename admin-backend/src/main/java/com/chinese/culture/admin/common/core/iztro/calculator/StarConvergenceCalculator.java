...
import com.chinese.culture.admin.common.core.iztro.data.enums.StarConvergenceType;
...
public class StarConvergenceCalculator {
     ...
     * @return 会合类型
     */
    public static StarConvergenceType calculateConvergenceType(StarBO star1, StarBO star2, PalaceBO palace1, PalaceBO palace2) {
        // 检查是否在同一宫位
        if (palace1 == palace2) {
     ...
     * @return 会合效果描述
     */
    public static String calculateConvergenceEffect(StarBO star1, StarBO star2, PalaceBO palace1, PalaceBO palace2) {
     ...
     * 获取会合效果描述
     */
    private static String getConvergenceEffect(StarBO star1, StarBO star2, StarConvergenceType convergenceType) {
     ...
     * 获取基础会合效果
     */
    private static String getBaseConvergenceEffect(StarBO star1, StarBO star2) {
     ...
     * @return 会合列表
     */
    public static List<Map<String, Object>> findAllConvergences(List<PalaceBO> palaces) {
        List<Map<String, Object>> convergences = new ArrayList<>();

        // 遍历所有宫位组合
        for (int i = 0; i < palaces.size(); i++) {
            PalaceBO palace1 = palaces.get(i);
            List<StarBO> stars1 = palace1.getAllStars();

            for (int j = i; j < palaces.size(); j++) {
                PalaceBO palace2 = palaces.get(j);
                List<StarBO> stars2 = palace2.getAllStars();

                // 检查两个宫位中的星耀会合
                for (StarBO star1 : stars1) {
                    for (StarBO star2 : stars2) {
                        // 避免自己和自己比较
