...
public class StarCombinationAnalyzer {
     ...
     * @return 三方四正组合
     */
    public static Map<String, List<String>> analyzeTrineAndOpposition(List<StarBO> stars, int position) {
        try {
     ...
     * @return 会合组合
     */
    public static Map<String, List<String>> analyzeStarConvergence(List<StarBO> stars) {
        try {
            Map<String, List<String>> result = new HashMap<>();
     ...
     * 获取指定位置的星耀
     */
    private static List<String> getStarsAtPosition(List<StarBO> stars, int position) {
        List<String> result = new ArrayList<>();
        for (StarBO star : stars) {
            if (star.getPosition() == position) {
                result.add(star.getName().getDescription());
            }
        }
        return result;
    }
