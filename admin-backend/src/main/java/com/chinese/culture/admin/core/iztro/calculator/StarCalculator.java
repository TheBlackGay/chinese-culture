...
public class StarCalculator {
    ...
    public static List<Star> calculateZiWeiStars(int lunarYear, int lunarMonth, int lunarDay, int birthHour) {
        ...
        // 计算天机星位置（紫微顺三）
        Star tianJi = new Star();
        tianJi.setName("天机");
        tianJi.setPosition((ziWeiPosition + 3) % 12);
        stars.add(tianJi);
        
        // 计算太阳星位置（天机顺六）
        Star taiYang = new Star();
        taiYang.setName("太阳");
        ...
        // 计算天同星位置（武曲顺一）
        Star tianTong = new Star();
        tianTong.setName("天同");
        tianTong.setPosition((ziWeiPosition + 2) % 12);
        stars.add(tianTong);
        
        // 计算廉贞星位置（天同顺六）
        Star lianZhen = new Star();
        lianZhen.setName("廉贞");
    ...
    public static List<Star> calculateTianFuStars(int lunarYear, int lunarMonth, int lunarDay, int birthHour) {
        ...
        // 计算太阴星位置（天府逆四）
        Star taiYin = new Star();
        taiYin.setName("太阴");
        taiYin.setPosition((tianFuPosition + 8) % 12);
        stars.add(taiYin);
        
        // 计算贪狼星位置（太阴逆二）
        Star tanLang = new Star();
        tanLang.setName("贪狼");
        ...
        // 计算天相星位置（巨门逆三）
        Star tianXiang = new Star();
        tianXiang.setName("天相");
        tianXiang.setPosition((tianFuPosition + 1) % 12);
        stars.add(tianXiang);
        
        // 计算天梁星位置（天相逆三）
        Star tianLiang = new Star();
        tianLiang.setName("天梁");
