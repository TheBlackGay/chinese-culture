 ...
 *    - 天干五行：每个天干对应一个五行属性
 *    - 地支五行：每个地支对应一个五行属性
 *    - 纳音五行：将干支组合纳入60甲子，对应不同形态的五行，如"海中金"、"炉中火"等
 * 
 * 3. 神煞系统：
 *    - 十神：用神、忌神等，反映天干之间的关系
 *      比肩、劫财：与日主同气
 ...
 * 
 * 4. 计算入口：
 *    - getLunarInfo(date: Date): 主要计算函数，返回包含所有信息的对象
 *    - getBaZi(date: Date): 获取八字数组
 *    - getNextTerm(date: Date): 获取下一个节气信息
 *    - getTrueSolarTime(date: Date): 获取真太阳时
 */
...
// 纳音五行对应表
// 纳音五行是将干支组合纳入60甲子中所对应的五行
// 例如：甲子年的纳音五行是"海中金"，表示这一年的五行特性
// 纳音五行分为：金、木、水、火、土五类，每类又有不同的形态
// 如：金有"剑锋金"、"白蜡金"等；木有"松柏木"、"杨柳木"等
const naYinWuXing: { [key: string]: string } = {
  '甲子': '海中金', '乙丑': '海中金', // 海中金：如海中之金，取其沉稳内敛之象
  '丙寅': '炉中火', '丁卯': '炉中火', // 炉中火：如炉中之火，取其温暖持久之象
  '戊辰': '大林木', '己巳': '大林木', // 大林木：如林中大树，取其高大挺拔之象
  '庚午': '路旁土', '辛未': '路旁土',
  '壬申': '剑锋金', '癸酉': '剑锋金',
  '甲戌': '山头火', '乙亥': '山头火',
  '丙子': '涧下水', '丁丑': '涧下水',
